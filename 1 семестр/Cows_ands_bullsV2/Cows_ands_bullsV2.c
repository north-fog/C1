...
#include <math.h>    // Библиотека основных матемаматичсеких функций
...
void main()
{
    ...
    int arr[5]={-1,-2,-3,-4,-5};        //Массив с загадаными числами
    int answer[5] ={-1,-1,-1,-1,-1};    //Массив с числами пользователя
    int size, p_num;                    //Переменая для длины числа и вспомогательная переменая
    int cow = 0, bull = 0, atempt = 0;  //Переменые коров, быков, попыток
    int test = 1;                       //Переход в режим разработчика (если test=0 то обычная игра, если test=1 то режим разработчика)
    int flag;                           //Переменая для цикла while  
    char ch;                            //Переменая для проверки ввода
    ...
    //Вывод фразы о победе в игре
    printf("Вы победили,сделав %d попыток. Игра окончена.",atempt);
